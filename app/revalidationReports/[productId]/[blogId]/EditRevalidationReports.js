...
export function EditRevalidationReports() {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `/api/revalidationReports/${productId}/${blogId}`,
        formData,
        {
          headers: {
      ...
      <div className="">
        {sections.map((section, sectionIndex) => (
          <div key={sectionIndex} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor={`status-${sectionIndex}`}>Status</Label>
              <Select
                onValueChange={(value) =>
                  handleSectionChange(sectionIndex, "status", value)
                }
